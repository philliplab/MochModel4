...
#run_scenario(td, available_scenarios[[1]]$scex, td, 'MochModel4.exe')
...
```{r Symptomatic Weibull Diagnosis Rates matches inputs, results='asis'}
...
diagnosed_mothers <- sum(diagnosis$`Number of new diagnosed mothers`)
ratio <- diagnosed_mothers / symptomatic_mothers
print(list('symptomatic mothers' = symptomatic_mothers,
           'diagnosed mothers' = diagnosed_mothers,
           'ratio' = ratio))
result <- ratio > 0.78 & ratio < 0.82
```
```{r, results='asis'}
test_results <- new_result(test_results,
                         name = ctest_name,
