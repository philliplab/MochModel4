...
test_results <- new_test(test_results,
                         name = ctest_name,
                         scenarios = cscenarios,
                         description = "A plot of the fertility rates of the mothers. TODO: Change this into a test")
```
```{r, warning=FALSE, message=FALSE}
mfr <- as.data.frame(new_RTable("MotherFertilityRates", db_channels[[cscenarios]]))
mfr <- subset(mfr, metrics == 'Fertility Rates')[,1:3]
p <- ggplot(mfr, aes(x = `Age`, y = `Value`)) + 
  geom_line() +
  ylab('Number of Births')
print(p)
result <- 'Manual'
